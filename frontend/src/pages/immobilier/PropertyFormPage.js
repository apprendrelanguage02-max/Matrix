...
import {
  ArrowLeft, Camera, Video, MapPin, Home, DollarSign, Phone,
  FileText, Check, Loader2, Save, Info
} from "lucide-react";
import {
  GNF_TO_USD, GNF_TO_EUR, CITIES, COMMUNES, CATEGORIES, OPERATIONS, EQUIPMENT,
  SectionCard, Label, Input, Select, NumInput, formatPrice,
  ImageGallery, PropertySummaryCard
...
export default function PropertyFormPage() {
  ...
  const setAsMain = (idx) => {
    setForm(f => { const imgs = [...f.images]; const [main] = imgs.splice(idx, 1); imgs.unshift(main); return { ...f, images: imgs }; });
  };
  const handleDragStart = (idx) => setDragIdx(idx);
  const handleDragOver = (e, idx) => {
    e.preventDefault();
    if (dragIdx === null || dragIdx === idx) return;
    setForm(f => { const imgs = [...f.images]; const [moved] = imgs.splice(dragIdx, 1); imgs.splice(idx, 0, moved); return { ...f, images: imgs }; });
    setDragIdx(idx);
  };
  ...
  const handleSubmit = async (asDraft = false) => {
    ...
    try {
      const payload = {
        title: form.title, type: form.type, price: parseFloat(form.price) || 0, currency: form.currency || "GNF",
        ...
        images: form.images, video_url: form.video_url, property_category: form.property_category,
        bedrooms: form.bedrooms || 0, bathrooms: form.bathrooms || 0, surface_area: parseFloat(form.surface_area) || 0,
        salons: form.salons || 0, kitchens: form.kitchens || 0, toilets: form.toilets || 0, floors: form.floors || 0,
        year_built: form.year_built, commune: form.commune, landmarks: form.landmarks, equipment: form.equipment,
        show_usd: form.show_usd, show_eur: form.show_eur, hide_email: form.hide_email, show_phone: form.show_phone,
    ...
    <div className="min-h-screen bg-[#F9FAFB] flex flex-col" data-testid="property-form-page">
      <div className="bg-white border-b border-zinc-200 shadow-sm sticky top-0 z-30">
        <div className="max-w-[1400px] mx-auto px-4 py-3 flex items-center gap-4">
          <Link to="/immobilier" className="flex items-center gap-2" data-testid="header-logo-link"><img src="/nimba-logo.png" alt="Matrix News" className="h-8" /></Link>
          ...
          <Link to="/dashboard" className="hidden sm:flex items-center gap-1.5 text-xs text-zinc-500 hover:text-[#F97316] font-bold uppercase tracking-wider transition-colors" data-testid="nav-dashboard"><Home className="w-3.5 h-3.5" /> Tableau de bord</Link>
          <Link to="/immobilier" className="hidden sm:flex items-center gap-1.5 text-xs text-zinc-500 hover:text-[#F97316] font-bold uppercase tracking-wider transition-colors" data-testid="nav-annonces"><FileText className="w-3.5 h-3.5" /> Mes annonces</Link>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-3 space-y-5">
            <SectionCard title="Photos du bien" icon={<Camera className="w-4 h-4 text-[#F97316]" />} id="section-photos">
              <ImageGallery images={form.images} uploading={uploading} fileInputRef={fileInputRef}
                handleImageUpload={handleImageUpload} removeImage={removeImage} setAsMain={setAsMain}
                handleDragStart={handleDragStart} handleDragOver={handleDragOver} handleDragEnd={handleDragEnd}
                dragIdx={dragIdx} errors={errors} />
            </SectionCard>
            <SectionCard title="Video (optionnel)" icon={<Video className="w-4 h-4 text-[#F97316]" />}>
              <Input value={form.video_url} onChange={e => set("video_url", e.target.value)}
                placeholder="https://youtube.com/watch?v=... ou URL directe" hint="Lien YouTube ou video MP4" data-testid="video-url" />
            </SectionCard>
          </div>
          ...
          <div className="lg:col-span-5 space-y-5">
            <SectionCard title="Informations du bien" icon={<Home className="w-4 h-4 text-[#F97316]" />}>
              <div className="space-y-4" data-field="title">
                <div>
                  <Input label="Titre de l'annonce" required value={form.title} onChange={e => set("title", e.target.value)}
                    placeholder="Ex: Villa moderne de 5 chambres a Kipe" className={errCls("title")} data-testid="prop-title" />
                  {errors.title && <p className="text-red-500 text-xs mt-1">{errors.title}</p>}
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <Select label="Type de bien" required value={form.property_category} onChange={e => set("property_category", e.target.value)} data-testid="prop-category">
                    {CATEGORIES.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
                  </Select>
                  <Select label="Type d'operation" required value={form.type} onChange={e => set("type", e.target.value)} data-testid="prop-type">
                ...
                {isEdit && (
                  <Select label="Statut" value={form.status} onChange={e => set("status", e.target.value)} data-testid="prop-status">
                    <option value="disponible">Disponible</option><option value="reserve">Reserve</option><option value="vendu">Vendu</option>
                  </Select>
                )}
              </div>
            </SectionCard>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" value={form.price ? formatPrice(form.price) : ""} data-testid="prop-price"
                      onChange={e => set("price", e.target.value.replace(/[^\d]/g, ""))} placeholder="Ex: 980 000 000"
                  ...
                  <div data-field="surface_area">
                    <Input label="Surface (m2)" required type="number" value={form.surface_area}
                      onChange={e => set("surface_area", e.target.value)} placeholder="320" className={errCls("surface_area")} data-testid="prop-surface" />
                  ...
                  <div className="bg-[#FFF7ED] border border-orange-200 rounded-md p-3 space-y-2">
                    <div className="flex items-center gap-3">
                      <label className="flex items-center gap-1.5 cursor-pointer">
                        <input type="checkbox" checked={form.show_usd} onChange={e => set("show_usd", e.target.checked)} className="accent-[#F97316] rounded" data-testid="show-usd" />
                        <span className="text-xs text-zinc-600 font-medium">USD</span>
                      </label>
                      <label className="flex items-center gap-1.5 cursor-pointer">
                        <input type="checkbox" checked={form.show_eur} onChange={e => set("show_eur", e.target.checked)} className="accent-[#F97316] rounded" data-testid="show-eur" />
            ...
            <SectionCard title="Details des pieces" icon={<Home className="w-4 h-4 text-[#F97316]" />}>
              ...
              <div className="mt-3">
                <Input label="Annee de construction" value={form.year_built} onChange={e => set("year_built", e.target.value)} placeholder="Ex: 2020" data-testid="prop-year" />
              </div>
            </SectionCard>
            ...
            <SectionCard title="Equipements et caracteristiques" icon={<Check className="w-4 h-4 text-[#F97316]" />}>
              <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
                      {items.map(item => (
                        <label key={item} className="flex items-center gap-2 cursor-pointer py-1 hover:bg-zinc-50 rounded px-1 transition-colors">
                          <input type="checkbox" checked={form.equipment.includes(item)} onChange={() => toggleEquip(item)}
                            className="accent-[#F97316] rounded" data-testid={`equip-${item.replace(/\s/g, "-")}`} />
                ...
                {form.equipment.length > 0 && <p className="text-xs text-[#F97316] font-medium">{form.equipment.length} equipement(s) selectionne(s)</p>}
              </div>
            </SectionCard>
            ...
            <SectionCard title="Informations de contact" icon={<Phone className="w-4 h-4 text-[#F97316]" />}>
              <div className="space-y-3" data-field="seller_name">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Input label="Nom du contact" required value={form.seller_name} onChange={e => set("seller_name", e.target.value)}
                  ...
                  <div data-field="seller_phone">
                    <Input label="Telephone" required value={form.seller_phone} onChange={e => set("seller_phone", e.target.value)}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Input label="Email" value={form.seller_email} type="email" onChange={e => set("seller_email", e.target.value)}
                    placeholder="email@exemple.com" data-testid="prop-seller-email" />
                  <Input label="WhatsApp" value={form.seller_whatsapp} onChange={e => set("seller_whatsapp", e.target.value)}
                ...
                <div className="flex flex-wrap gap-4 pt-1">
                  <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-600">
                    <input type="checkbox" checked={form.show_phone} onChange={e => set("show_phone", e.target.checked)} className="accent-[#F97316]" /> Afficher le telephone
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-600">
                    <input type="checkbox" checked={form.hide_email} onChange={e => set("hide_email", e.target.checked)} className="accent-[#F97316]" /> Masquer l'email
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-600">
                    <input type="checkbox" checked={form.whatsapp_direct} onChange={e => set("whatsapp_direct", e.target.checked)} className="accent-[#F97316]" /> Contact WhatsApp direct
                  </label>
                </div>
              </div>
            </SectionCard>
          ...
          <div className="lg:col-span-4 space-y-5">
            <SectionCard title="Localisation" icon={<MapPin className="w-4 h-4 text-[#F97316]" />}>
              <div className="space-y-3" data-field="city">
                ...
                <Input label="Quartier / Secteur" value={form.neighborhood} onChange={e => set("neighborhood", e.target.value)}
                  placeholder="Ex: Kipe, Cosa, Nongo..." data-testid="prop-neighborhood" />
                <Input label="Adresse complete" value={form.address} onChange={e => set("address", e.target.value)}
                  placeholder="Rue, numero, immeuble..." data-testid="prop-address" />
                <div>
                  <Label>Points de repere</Label>
                  <textarea value={form.landmarks} onChange={e => set("landmarks", e.target.value)} rows={2}
                    placeholder="Ex: A proximite du rond-point de Kipe..."
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Input label="Latitude GPS" value={form.latitude} onChange={e => set("latitude", e.target.value)} placeholder="9.5370" data-testid="prop-lat" />
                  <Input label="Longitude GPS" value={form.longitude} onChange={e => set("longitude", e.target.value)} placeholder="-13.6785" data-testid="prop-lng" />
                </div>
              </div>
            </SectionCard>
            ...
            <SectionCard title="Carte interactive" icon={<MapPin className="w-4 h-4 text-[#F97316]" />}>
              <div className="rounded-md overflow-hidden border border-zinc-200" style={{ height: 350 }} data-testid="prop-map">
                <Suspense fallback={<div className="w-full h-full flex items-center justify-center bg-zinc-100"><Loader2 className="w-5 h-5 animate-spin text-[#F97316]" /></div>}>
                  <MapWithPin lat={parseFloat(form.latitude) || 9.537} lng={parseFloat(form.longitude) || -13.6785} onMove={handleMapClick} />
                </Suspense>
              </div>
            ...
            <PropertySummaryCard form={form} priceNum={priceNum} />
          </div>
        </div>
